...
def engineer_reviews(df, contractions_path, slangs_path):
    ...
    df['cleaned_incentivized_review'] = check_incentivized_text(df['cleaned_text'], contractions_path, slangs_path, df['cleaned_incentivized_review'])
...
def engineer_review_activity(df, contractions_path, slangs_path):
    ...
    # engineer incentivized reviews
    df['cleaned_incentivized_review'] = [0] * len(df)
    df['cleaned_incentivized_review'] = check_incentivized_text(temp_new_text(df['cleaned_text']), contractions_path, slangs_path, df['cleaned_incentivized_review'])

    # engineer loreal reviews
    df['cleaned_loreal_review'] = [0] * len(df)
    df['cleaned_loreal_review'] = check_loreal_reviews(temp_new_text(df['cleaned_text']), contractions_path, slangs_path, df['cleaned_loreal_review'])
